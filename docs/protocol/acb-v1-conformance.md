...
- `minimal.canonical.txt` is the exact UTF-8 byte sequence an ACB v1 canonical
  encoder must emit for that manifest. It intentionally has no trailing newline.

The fixture includes one conversation event and one text resource containing
the six bytes `hello\n`.
...
1. Parse `minimal.json` as `acb-v1` and reject an unknown `version` rather than
   guessing how to interpret it.
2. Decode every resource's canonical Base64 payload.
3. Verify each resource's declared byte length and SHA-256 before accepting the
   bundle.
4. Canonically encode the validated manifest with recursively sorted object
   keys, array order preserved, no insignificant whitespace, and UTF-8 output.
...
7. Compute the logical fingerprint from the logical fields defined by ACB v1 and
   compare it with the expected fingerprint above. `exportedAt` and resource
   `contentBase64` are intentionally excluded from that fingerprint; resource
   identity and integrity metadata remain included.
