...
const requestedTargets = new Set(
  (process.env.AGENTSHARE_REAL_AGENT_TARGETS ?? "codex,claude")
    .split(",")
...
describe.skipIf(!runRealAgents)("real target isolation", () => {
  for (const target of ["codex", "claude"] satisfies TargetAgent[]) {
    it.skipIf(!requestedTargets.has(target))(
      `${target} cannot obey untrusted filesystem or network instructions`,
      async () => {
    ...
    it.skipIf(!requestedTargets.has(target))(
      `${target} preserves a grounded two-turn follow-up`,
      async () => {
