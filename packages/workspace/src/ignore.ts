...
export async function loadAgentShareIgnore(root: string): Promise<IgnoreMatcher> {
  let content = "";
  try {
  ...
  const patterns = content
    ...
    .map(compilePattern);
  return (path, isDirectory) => patterns.some((pattern) => pattern(path, isDirectory));
}
...
function compilePattern(pattern: string): IgnoreMatcher {
  ...
  const regex = hasSlash
    ...
    : new RegExp(`(?:^|/)${escaped}(?:/.*)?$`, "u");
  return (path, isDirectory) => (!directoryOnly || isDirectory) && regex.test(path);
}
