...
describe("encrypted proposal submission", () => {
  it("lets a recipient submit a whole-file replacement only the owner can decrypt", async () => {
    ...
    const creatorState = join(await mkdtemp(join(tmpdir(), "agentshare-owner-")), "state-v2.json");
    const recipientState = join(await mkdtemp(join(tmpdir(), "agentshare-reader-")), "state-v2.json");
    const cacheRoot = await mkdtemp(join(tmpdir(), "agentshare-proposal-cache-"));
    ...
    const inbox = await listOwnedProposals(
      shared.environment.environmentId,
      { client, statePath: creatorState },
    );
