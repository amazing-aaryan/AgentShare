...
async function fixture(): Promise<string> {
  ...
  try {
    ...
    // Windows CI may not grant symlink privileges. The production code still rejects them.
  }
  return root;
}
...
describe("buildWorkspaceSnapshot", () => {
  it("includes safe regular files with normalized paths and hashes", async () => {
    ...
    expect(snapshot.files.every((file) => /^[a-f0-9]{64}$/u.test(file.sha256))).toBe(true);
  ...
  it("never includes AgentShare, git, dependencies, credentials, or symlinks", async () => {
    const root = await fixture();
    const snapshot = await buildWorkspaceSnapshot(root, { preferGit: false });
    expect(snapshot.files.some((file) => /(?:\.git|\.agentshare|node_modules|\.env|secret-link)/u.test(file.path))).toBe(false);
    expect(snapshot.excluded.some((entry) => entry.reason === "credential-policy")).toBe(true);
  });
});
