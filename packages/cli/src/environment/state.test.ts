...
describe("environment state v2", () => {
  it("persists owner and recipient capabilities separately", async () => {
    ...
    await saveOwnedEnvironment({
      ...
      sharePolicy: { includeConversation: true, includeWorkspace: true, proposalsEnabled: true },
    }, path);
    ...
    expect((await findOwnedEnvironmentForWorkspace("/workspace/project", path))?.updateCapability).toBe("u".repeat(43));
    expect((await findAttachedEnvironment("env_other_12345678901234567", path))?.readCapability).toBe("q".repeat(43));
