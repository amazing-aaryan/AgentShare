...
import { createEnvironmentFromCapture, publishEnvironmentRevision } from "./publication.js";
...
describe("attached environment refresh", () => {
  it("advances the local encrypted cache when the same environment gets a new revision", async () => {
    ...
    expect(await readAttachedFile(created.environment.environmentId, "value.txt", { statePath, cacheRoot })).toBe("one\n");
    ...
    expect(await readAttachedFile(created.environment.environmentId, "value.txt", { statePath, cacheRoot })).toBe("two\n");
  });
});
