...
export async function buildWorkspaceSnapshot(
  ...
  const enumeration = await enumerateWorkspace(root, { preferGit: options.preferGit });
  ...
  for (const path of enumeration.files) {
    ...
    if (!metadata.isFile() || metadata.isSymbolicLink()) {
      excluded.push({ path, reason: metadata.isSymbolicLink() ? "symlink" : "unsupported-file-type" });
      continue;
    }
    ...
    if (resolved !== absolute && process.platform !== "win32") {
      // The enumerator already proves containment. This preserves the actual file path
      // while avoiding dereferencing a changed symlink between enumeration and read.
