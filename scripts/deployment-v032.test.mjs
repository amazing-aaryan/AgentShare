...
test(
  "v0.3.2 production gate keeps immutable candidate bytes separate from reviewed release control",
  async () => {
...
test(
  "v0.3.2 production gate deploys the changed handoff and keeps the relay unchanged",
  async () => {
    const workflow = await workflowSource();
    ...
    assert.match(
      workflow,
      /packages\/cli\/src\/public-handoff\.e2e\.test\.ts/u,
    );
    assert.match(
      workflow,
